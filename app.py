...
@app.route('/weather-suggestion',methods=['GET','POST'])
def weather_suggestion():
    ...
    if request.method == 'POST':
        ...
        weather_data = []
        dates = set()  # Keep track of unique dates
        for forecast in data['list']:
            date = forecast['dt_txt'].split()[0]  # Extract date without the time
...
def fert_recommend():
    ...
    return render_template('fertilizer-result.html', recommendation=response, prediction= final_prediction, title=title)
